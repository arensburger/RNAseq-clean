...
my $READLEN = 150; # maximum read length this is for merging the reads
my $MINLEN = 35;
...
print LOG datetime, " File $readspair2, total FASTQ reads: ", count_fastq($readspair2, "fq"), "\n"; # do a basic count
print LOG "\n";

#clip adapters
print LOG datetime, " Clipping adapters and quality filter with Trimmomatic\n";
clipadapters($readspair1, $readspair2);
print LOG datetime, " File with paired reads, FASTQ reads: ", count_fastq($paired_output, "fq"), "\n"; 
print LOG datetime, " File with unpaired reads, FASTQ reads: ", count_fastq($unpaired_output, "fq"), "\n";
print LOG "\n";

#merge overlaping pairs
print LOG datetime, " Merging overlaping pairs with FLASH, putting merged into unpaired file\n";
mergeoverlaping($paired_output);
print LOG datetime, " File with paired reads, FASTQ reads: ", count_fastq($paired_output, "fq"), "\n"; 
print LOG datetime, " File with unpaired reads, FASTQ reads: ", count_fastq($unpaired_output, "fq"), "\n";
print LOG "\n";
...
print LOG datetime, " File with unpaired reads, FASTA reads: ", count_fastq($unpaired_output, "fa"), "\n";

#print the data files
my $pairedoutname = $outputdir . "/" . $outputname .  "-paired.fq"; 
my $unpairedoutname = $outputdir . "/" . $outputname .  "-unpaired.fa";
...
# Oct 2015, stopped printing this because if one member of the pair is id'ed as a ribosome it's likely
# the other member is a ribosome too
#				print OUTPUT2 "$ref1\n";
#				print OUTPUT2 $ref2;
				$ref1 = $l1;
				$ref2 = $l2;
...
sub mergeoverlaping {
	my ($inputfile) = @_;
	my $tempdir = tempdir(); # temporary directory to put ouput in
	my $MIN_OVERLAP=30; # amount of overlap to merge the paired reads
	`flash --interleaved-input -m $MIN_OVERLAP -M $READLEN -d $tempdir --interleaved-output $inputfile`;

	# put the results of the merging into the proper files
	my $tempfile = File::Temp->new( UNLINK => 1, SUFFIX => '.fastq' );
	`cat $unpaired_output $tempdir/out.extendedFrags.fastq > $tempfile`;
	`cp $tempfile $unpaired_output`; 
