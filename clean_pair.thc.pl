...
# Changed the TMP files to the PATH to correct for local installions. Also corrected assembly artifacts for no unpaired reads.
...
my $PATH = "/home/clarket/RNASeq-clean/RNAseq-clean-master";
my $TRIMMOMATIC_PATH = "$PATH/Trimmomatic-0.20";
my $RIBOSOME_BOWTIE2_FILE = "$PATH/arthropod_ribosomes";
my $FASTX_TOOLKIT_PATH = "/export/home/arensburgerp/bin/";
my $MINLEN = 26;
...
my $paired_output = File::Temp->new( UNLINK => 1, SUFFIX => '.fastq', DIR => $PATH ); # temporary file with chastity results pair1
my $unpaired_output = File::Temp->new( UNLINK => 1, SUFFIX => '.fastq', DIR => $PATH ); # temporary file with chastity results pair1
print STDERR "A\n";
#create log file
if ( -e $outputdir) {
...
#artifact filter
if (count_fastq($unpaired_output) > 0)
{
...
{
    print LOG datetime, " No unpaired reads. Skipping artifact removal...\n";
}
...
sub stats1 {
	...
	my $ts = File::Temp->new( UNLINK => 1, SUFFIX => '.txt', DIR => $PATH ); # temporary file
...
sub filter_chastity_pair {
	...
	open (INPUT, $infile) or die "cannot open file $infile\n";
        my $chasoutput = File::Temp->new( UNLINK => 1, SUFFIX => '.fastq', DIR=>$PATH );
...
#	print STDERRR "clipping adapters...\n";	
	...
	my $forward_paired = File::Temp->new( UNLINK => 1, SUFFIX => '.fastq', DIR => $PATH ) or die(); 
	my $reverse_paired = File::Temp->new( UNLINK => 1, SUFFIX => '.fastq', DIR => $PATH); 
	my $forward_unpaired = File::Temp->new( UNLINK => 1, SUFFIX => '.fastq', DIR => $PATH );
	my $reverse_unpaired = File::Temp->new( UNLINK => 1, SUFFIX => '.fastq', DIR => $PATH ); 
	`java -classpath $TRIMMOMATIC_PATH/trimmomatic-0.20.jar org.usadellab.trimmomatic.TrimmomaticPE -threads $threads -phred33 $inputfile1 $inputfile2 $forward_paired $forward_unpaired $reverse_paired $reverse_unpaired LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:$MINLEN`;
...
sub ribosome_removal{
	...
	open (INPUT1, $infile) or die "cannot open file $infile\n";
        my $ribooutput = File::Temp->new( UNLINK => 1, SUFFIX => '.fastq', DIR=>$PATH );
	open (OUTPUT1, ">$ribooutput") or die;

	my $tx = File::Temp->new( UNLINK => 1, SUFFIX => '.sam', DIR => $PATH ); # temporary file with hits
...
sub filter_artifact {
	my ($inputfile) = @_;
	my $tx = File::Temp->new( UNLINK => 1, SUFFIX => '.fastq', DIR => $PATH ); # temporary file with hits
	print LOG "Starting...\n";
	my $wctext = `wc -m $inputfile`;
	print LOG "$wctext--\n";
